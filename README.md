...
<sup>4</sup> Claude Chat is a live, pull-only source. On macOS, explicitly opt into the signed-in Claude Desktop session with `TXCRIPT_CLAUDE_CHAT_AUTH=desktop`; environment-supplied session and Cloudflare credentials are rejected in V1. An optional `TXCRIPT_CLAUDE_CHAT_ORGANIZATION_UUID` restricts discovery, while Desktop auth otherwise uses the app's active organization. Before enumerating conversations, txcript warns that discovery uses an undocumented private endpoint Anthropic can observe or restrict. txcript performs GET requests only and refuses save, delete, same-harness continue, and `--with claude_chat`. Generated files presented on the active branch become Common artifact blocks; Claude Code conversions materialize them beside the generated session and use Claude Code's native `Artifact` tool. Claude's data-export ZIP and `conversations.json` are not supported.
