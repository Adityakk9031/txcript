...
- **Search everything**: literal search across every session on the machine, case-insensitive by default, with fzf-style fuzzy matching available (powered by [nucleo](https://github.com/helix-editor/nucleo)) — as a library API, a one-shot CLI query, or an interactive picker.
...
A pattern matches literally and case-insensitively: `relay bug` finds lines containing that exact text, spaces and all. Pass `--fuzzy` for fzf-style matching instead — space-separated terms that must all appear, plus `'exact`, `^prefix`, `suffix$`, and `!not`. Fuzzy reaches further but matches any line whose characters contain the pattern in order, so it turns up lines that only look like matches.
