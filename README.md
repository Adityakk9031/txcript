...
<sup>4</sup> Claude Chat is a live, pull-only source. Set `TXCRIPT_CLAUDE_CHAT_SESSION_KEY`, or explicitly opt into the signed-in macOS Claude Desktop cookie with `TXCRIPT_CLAUDE_CHAT_AUTH=desktop`. An optional `TXCRIPT_CLAUDE_CHAT_ORGANIZATION_UUID` restricts discovery and can bypass a rejected account-wide organization lookup; Desktop auth automatically uses the app's active organization. txcript performs GET requests only and refuses save, delete, same-harness continue, and `--with claude_chat`. Generated files presented on the active branch become Common artifact blocks; Claude Code conversions materialize them beside the generated session and use Claude Code's native `Artifact` tool. Claude's data-export ZIP and `conversations.json` are not supported.
